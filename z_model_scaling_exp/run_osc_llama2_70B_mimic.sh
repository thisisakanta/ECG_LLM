...
#SBATCH --job-name=70B_m       # 作业名称
#SBATCH --account=PAS2473		    # Project ID
#SBATCH --output=/users/PAS2473/brucewan666/ECG/ECG/output_logs/70B_m_ecg.log        # 输出日志文件
#SBATCH --error=/users/PAS2473/brucewan666/ECG/ECG/output_logs/70B_m_error.log         # 错误日志文件
#SBATCH --nodes=1                   # 节点数
#SBATCH --ntasks-per-node=1         # 每个节点的任务数
#SBATCH --cpus-per-task=4           # 每个任务使用的 CPU 核心数
#SBATCH --gpus-per-node=1        # GPU per node
#SBATCH --mem=128G                   # 内存限制
#SBATCH --time=80:00:00             # 作业运行时间限制
...
# Lora training
accelerate launch --main_process_port 31235 \
    ...
    --weight_decay 0. \
    --num_train_epochs 3 \
    --output_dir /fs/scratch/PAS2473/scaling_model_test/70B_mimic \
    --with_tracking \
