...
class ECGCLIP(torch.nn.Module):
    ...
    def ext_ecg_emb(self, ecg):
        """Return cross-modal ECG embedding (projected) without normalization.
        Backwards-compatible existing method used by zeroshot scripts.
        Prefer get_ecg_features(project=True, normalize=True) for explicit control.
        """
        if 'resnet' in self.ecg_model:
            ecg_backbone = self.ecg_encoder(ecg)
            ecg_backbone = self.downconv(ecg_backbone)
            proj_ecg_emb, _ = self.att_pool_head(ecg_backbone)
            proj_ecg_emb = proj_ecg_emb.view(proj_ecg_emb.shape[0], -1)
        elif 'vit' in self.ecg_model:
            ecg_backbone = self.ecg_encoder(ecg)
            proj_ecg_emb = self.proj_e(ecg_backbone)
        else:
            raise ValueError(f"Unsupported ecg_model type: {self.ecg_model}")
        return proj_ecg_emb

    @torch.no_grad()
    def get_ecg_features(self, ecg, project=True, l2_normalize=True):
        """Return ECG features.
        Args:
            ecg: Tensor of shape (B, C, L)
            project: If True, apply projection head / attention pooling to map into joint space.
            l2_normalize: If True, L2 normalize the output.
        Returns:
            Tensor of shape (B, D)
        """
        ##    here only the ecg encoder part is changed  ##
        if 'resnet' in self.ecg_model:
            x = self.ecg_encoder(ecg)          # (B, feat_dim, L')
            x = self.downconv(x)               # channel align
            if project:
                x, _ = self.att_pool_head(x)   # (B, 1, D') -> squeeze handled inside
                x = x.view(x.shape[0], -1)
            else:
                # average pool raw feature for a stable unprojected representation
                x = self.avgpool(x).view(x.shape[0], -1)
        elif 'vit' in self.ecg_model:
            x = self.ecg_encoder(ecg)          # (B, width)
            if project:
                x = self.proj_e(x)             # (B, proj_out)
        else:
            raise ValueError(f"Unsupported ecg_model type: {self.ecg_model}")
        if l2_normalize:
    ...
    @torch.no_grad()
    def get_text_features(self, input_ids, attention_mask, project=True, l2_normalize=True):
        """Return text features in raw or projected joint space.
        Args:
            input_ids: (B, T)
            attention_mask: (B, T)
            project: If True apply self.proj_t
            l2_normalize: If True L2 normalize
        Returns:
            Tensor (B, D)
        """
        ### here only the text encoder part is changed  ###
        h = self.get_text_emb(input_ids, attention_mask)  # raw pooled output
        if project:
            h = self.proj_t(h.contiguous())
        if l2_normalize:
