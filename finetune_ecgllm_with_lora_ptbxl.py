...
def parse_args():
    ...
    parser.add_argument(
        "--val_data_path",
        type=str,
            default='/home/user/Downloads/2005047/MEIT/ECG_LLMs/ptbxl/ptbxl_ecg_val.jsonl',
            help=(
            "val_data_path"
        ),
    )
    ...
    parser.add_argument(
        "--test_data_path",
        type=str,
        default='/home/user/Downloads/2005047/MEIT/ECG_LLMs/ptbxl/ptbxl_ecg_test.jsonl',
        help=(
            "test_data_path"
        ),
    )
    parser.add_argument(
        "--ssl_pretrained_path",
        type=str,
         default="/home/user/Downloads/2005047/MEIT/ECG_LLMs/vit_tiny_best_ckpt.pth",
       
        help="Path to ECGCLIP SSL checkpoint (if provided, use it to init ECG_model).",
    )
    parser.add_argument(
            "--ssl_ecg_model",
            type=str,
            default="vit_tiny",  # or vit_*
            #default=None,   
            help="Backbone name used in ECGCLIP (must match the SSL checkpoint).",
    )
...
def main():
    ...
    def ecg_files_exist(example):
        # The original JSON has 'ecg_path' (because encode_with_messages_format uses example['ecg_path'])
        # If you changed it to 'ecg' in the raw json, swap the key accordingly.
        path = example.get("ecg_path") or example.get("ecg")
        if path is None:
            return False

        # PTB-XL stores .hea and .dat with the same base name
        hea_path = path + ".hea"
        dat_path = path + ".dat"

        return os.path.exists(hea_path) and os.path.exists(dat_path)

    # This will drop any train/val/test example whose ECG files are missing
...
#         # --- FIXED special-token logic for GPT-2 / OPT / NeoX / LLaMA ---
# # This block prevents _eos_token_tensor errors, isin() crashes, and right-padding bugs.
...
#         # OPT uses GPT-2 tokenizer but already has pad behavior
...
#         # GPT-2 (your ECG-LLM backbone)
#         elif isinstance(tokenizer, GPT2Tokenizer):
#             num_added_tokens = tokenizer.add_special_tokens({
#                 "unk_token": "<unk>",
#                 "pad_token": "<pad>"  # IMPORTANT: do NOT use "[PAD]" — use "<pad>" to match HF conventions
#             })
...
#         #  Must resize embeddings after ANY added token
