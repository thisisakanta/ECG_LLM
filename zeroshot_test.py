...
def main():
    ...
    if args.val_test_ratio > 1e-6:
        split_train_datasets = raw_datasets['train'].train_test_split(args.val_test_ratio)
    ...
    if "prompt" in split_train_datasets["train"].column_names and "completion" in split_train_datasets["train"].column_names:
        encode_function = partial(
            encode_with_prompt_completion_format,
            tokenizer=tokenizer,
            max_seq_length=args.max_seq_length,
        )
    elif "messages" in split_train_datasets["train"].column_names:
        encode_function = partial(
    ...
    with accelerator.main_process_first():  
        train_lm_datasets = split_train_datasets.map(
            encode_function,
            batched=False,
            num_proc=args.preprocessing_num_workers,                  ##################### modified test here for bug #################
            load_from_cache_file=not args.overwrite_cache,         
            remove_columns=[name for name in split_train_datasets["train"].column_names if name not in ["input_ids", "labels", "attention_mask", "ecg"]],
