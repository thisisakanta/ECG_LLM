...
class GPTNeoSelfAttention(nn.Module):
    ...
    def _attn(self, query, key, value, ecg_emb=None, attention_mask=None, head_mask=None):
        ...
        query = query.to(torch.float32) # torch.Size([16, 16, 75, 128]) # torch.Size([16, 16, 75, 128]) # torch.Size([16, 16, 75, 128])
        key = key.to(torch.float32)

        attn_weights = torch.matmul(query, key.transpose(-1, -2)) # torch.Size([16, 16, 75, 75])
