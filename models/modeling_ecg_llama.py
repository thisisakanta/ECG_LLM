...
from transformers.models.llama.modeling_llama import _get_unpad_data, _expand_mask, _make_causal_mask, \
LlamaRMSNorm, LlamaRotaryEmbedding, LlamaLinearScalingRotaryEmbedding, LlamaDynamicNTKScalingRotaryEmbedding, LlamaMLP
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        ecg_emb = None, # batch llama_head_dim
        **kwargs,
        ...
        # print('################### self.k_proj.weight', self.k_proj.weight[0, :20])
        ...
        if ecg_emb is not None: # ecg_emb = (batch, 128) -> (batch, 32, 128)
            ...
            # print('ecg_emb change?: #########', ecg_emb.shape)
            ...
            ecg_key_states = self.k_proj(ecg_hidden_states)
            ecg_value_states = self.v_proj(ecg_hidden_states)

           #  (batch,1, 32, 128)
            ecg_key_states = ecg_key_states.view(bsz, ecg_hidden_states.shape[1], self.num_heads, self.head_dim).transpose(1, 2)
            ecg_value_states = ecg_value_states.view(bsz, ecg_hidden_states.shape[1], self.num_heads, self.head_dim).transpose(1, 2)

            ecg_key_states = repeat_kv(ecg_key_states, self.num_key_value_groups)
            ecg_value_states = repeat_kv(ecg_value_states, self.num_key_value_groups) # (batch,170, 32, 128)


            combined_value_states = torch.concat(
                            [ecg_value_states.to(value_states.dtype), value_states],
                            dim=-2,
                        )
   
            
            ecg_attn_weights = torch.matmul(query_states, ecg_key_states.transpose(2, 3).to(key_states.dtype)) / math.sqrt(self.head_dim) # batch * 128 * 1
