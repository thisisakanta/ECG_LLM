...
class OPTAttention(nn.Module):
    ...
    def forward(
        ...
        if ecg_emb is not None: # ecg_emb = (batch, 128) -> (batch, 32, 128)
            ...
            ecg_key_states = self._shape(self.k_proj(ecg_hidden_states), -1, bsz)
            ecg_value_states = self._shape(self.v_proj(ecg_hidden_states), -1, bsz)

            proj_shape = (bsz * self.num_heads, -1, self.head_dim)

            ecg_key_states = ecg_key_states.view(*proj_shape)
            ecg_value_states = ecg_value_states.view(*proj_shape)
            ...
            ecg_attn_weights = torch.bmm(query_states, ecg_key_states.transpose(1, 2))
        ...
        if ecg_emb is not None:
            ...
            attn_output = torch.bmm(attn_probs, combined_value_states)
